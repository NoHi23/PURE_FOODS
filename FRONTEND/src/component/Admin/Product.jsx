...
const Product = () => {
  ...
  const salePrice = (editForm.price || 0) * (1 - (editForm.discountPercent || 0) / 100);
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      const errorMessage =
        err.response && err.response.data && err.response.data.message ? err.response.data.message : "ERROR";
      toast.error(errorMessage);
    }
  };

  useEffect(() => {
    axios.get("http://localhost:8082/PureFoods/api/product/getAll").then((res) => {
      setProducts(res.data.listProduct);
    });
  }, []);
  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      ...
      await Promise.all(
        uniqueSuppliersIds.map(async (id) => {
          try {
                      ...
                      <div class="title-header option-title d-sm-flex d-block">
                        ...
                        <div class="right-options">
                          <ul>
                            ...
                            <li>
                              <Link to={"/admin-add-new-product"} className="btn btn-solid">
                                Thêm mới sản phẩm
                              </Link>
                            </li>
                          </ul>
                                ...
                                <tr key={i}>
                                  <td>
                                    <div className="table-image">
                                      <img src={p.imageURL} className="img-fluid" alt="" />
                                    </div>
                                  </td>
                                  ...
                                  <td className="td-price">${p.salePrice != null ? p.salePrice.toFixed(2) : "0.00"}</td>
                                  <td>
                                    {p.harvestDate ? new Date(p.harvestDate).toLocaleDateString("vi-VN") : "Không rõ"}
                                  </td>
                                  <td>
                                    {p.expirationDate
                      ...
                      <div>
                        <div className="row">
                          ...
                          <div className="col-md-8">
                            ...
                            <p>
                              <strong>Loại:</strong> {categoryNames[selectedProduct.categoryId]}
                            </p>
                            <p>
                              <strong>Nhà cung cấp:</strong> {suppliersName[selectedProduct.supplierId]}
                            </p>
                            <p>
                              <strong>Số lượng:</strong> {selectedProduct.stockQuantity}
                            </p>
                            <p>
                              <strong>Giá:</strong> ${selectedProduct.price}
                            </p>
                            <p>
                              <strong>Phần trăm giảm giá:</strong> {selectedProduct.discountPercent || 0}%
                            </p>
                            <p>
                              <strong>Giá sau khi giảm:</strong> $
                              {selectedProduct.salePrice != null ? selectedProduct.salePrice.toFixed(2) : "0.00"}
                            </p>
                            <p>
                            ...
                            <p>
                              ...
                              {selectedProduct.harvestDate
                                ? new Date(selectedProduct.harvestDate).toLocaleDateString("vi-VN")
                                : "Không rõ"}
                            </p>
                            <p>
                              <strong>Ngày hết hạn:</strong>{" "}
                              {selectedProduct.expirationDate
                                ? new Date(selectedProduct.expirationDate).toLocaleDateString("vi-VN")
                                : "Không rõ"}
                            </p>

                            <p>
                              <strong>Trạng thái:</strong> {selectedProduct.status === 1 ? "Riêng tu" : "Công khai"}
                            </p>
                          </div>
                        </div>
                      </div>
                ...
                <div className="modal-content">
                  ...
                  <div className="modal-body">
                    <form>
                      ...
                      <div className="mb-3">
                        <label className="form-label">Discount: {editForm.discountPercent || 0}%</label>
                      ...
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          className="form-control"
                          name="harvestDate"
                          value={editForm.harvestDate ? String(editForm.harvestDate).slice(0, 10) : ""}
                          onChange={handleInputChange}
                        />
                      </div>
                      ...
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          className="form-control"
                          name="expirationDate"
                          value={editForm.expirationDate ? String(editForm.expirationDate).slice(0, 10) : ""}
                          onChange={handleInputChange}
                        />
                      </div>
          ...
          <div className="modal-content">
            <div className="modal-body">
              ...
              <div className="button-box">
                ...
                <button type="button" onclick="location.href = 'login.html';" className="btn  btn--yes btn-primary">
                  Yes
                </button>
              </div>
            </div>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <div className="remove-box">
                <p>
                  Bạn có chắc chắn muốn cấm sản phẩm này được không khai? Nó sẽ ảnh hưởng ít nhiều đến hệ thống của bạn!
                </p>
              </div>
            </div>
            <div className="modal-footer">
